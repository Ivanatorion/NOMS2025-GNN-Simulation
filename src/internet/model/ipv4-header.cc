...
Ipv4Header::Serialize (Buffer::Iterator start) const
{
  ...
  /*
  i.WriteU8 (m_gnn_identifier);
  i.WriteHtonU32 (m_gnn_iter);

  for(int x = 0; x < 16; x++){
    uint64_t aux;
    memcpy(&aux, &m_gnn_hiddenState[x], sizeof(double));
    i.WriteHtonU64 (aux);
  }
  */
}
...
Ipv4Header::Deserialize (Buffer::Iterator start)
{
  ...
  uint16_t headerSize = m_headerSize;
  ...
  /*
  m_gnn_identifier = i.ReadU8 ();
  m_gnn_iter = i.ReadNtohU32 ();

  for(int x = 0; x < 16; x++){
    uint64_t aux = i.ReadNtohU64 ();
    memcpy(&m_gnn_hiddenState[x], &aux, sizeof(double));
  }
  */
